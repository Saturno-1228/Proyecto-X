...
public class NPCController
{
	...
	public bool update(GameTime time, GameLocation location, List<NPCController> allControllers)
	{
		...
		foreach (NPCController n in allControllers)
		{
			...
			if (n.puppet.Equals(puppet))
			{
				reachedMeYet = true;
			}
			if (n.puppet.FacingDirection == puppet.FacingDirection && !n.puppet.Equals(puppet) && n.puppet.GetBoundingBox().Intersects(puppet.nextPosition(puppet.FacingDirection)))
			{
				if (reachedMeYet)
				{
					break;
