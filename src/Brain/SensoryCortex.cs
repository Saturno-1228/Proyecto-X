...
namespace StardewLivingValley.Brain
{
    public class SensoryCortex
    {
        ...
        public SensoryCortex(IMonitor logger, string modDirPath)
        {
            _logger = logger;
            _observationDirPath = Path.Combine(modDirPath, "data", "Brains");
            Directory.CreateDirectory(_observationDirPath);
        }

        private ObservationLog GetObservationLog(string npcName)
        {
            string npcDir = Path.Combine(_observationDirPath, npcName);
            string path = Path.Combine(npcDir, "observations.json");
            if (File.Exists(path))
            {
                try
                {
