...
namespace StardewLivingValley.Services
{
    ...
    public class EmotionService
    {
        ...
        public EmotionService(IModHelper helper, IMonitor logger)
        {
            _helper = helper;
            _logger = logger;
            LoadProfiles();
            ...
            if (File.Exists(path))
            {
                try
                {
                    ...
                    if (root != null && root.Npcs != null)
                    {
                        ...
                        _logger.Log($"[EmotionService] Cargados perfiles de emoción para {_profiles.Count} NPCs.", LogLevel.Trace);
                    }
                }
                catch (Exception ex)
                {
                    _logger.Log($"[EmotionService] Error leyendo portrait-emotion-profiles.json: {ex.Message}", LogLevel.Error);
                }
            }
            else
            {
                _logger.Log("[EmotionService] No se encontró Assets/portrait-emotion-profiles.json.", LogLevel.Warn);
            }
        }
